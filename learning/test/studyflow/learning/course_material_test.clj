...
<table><tr><td>_INPUT_1_ en </td><td><i> text _INPUT_2_ text</i></td></tr></table></p>
        ...
        text (str pre svg0 svg1 post)
        input-names #{"_INPUT_1_" "_INPUT_2_"}]
    (is (not= svg0 svg1))
    (is (= (material/text-with-inputs-to-tree text input-names)
           {:tag :div, :attrs nil, :content [{:tag :p, :attrs nil, :content ["some text before " {:tag :b, :attrs nil, :content ["with " {:tag :u, :attrs nil, :content ["HTML"]}]} "Met inline inputs" {:tag :svg, :attrs {:name "_SVG_0_"}, :content svg0} {:tag :svg, :attrs {:name "_SVG_1_"}, :content svg1} "\n" {:tag :table, :attrs nil, :content [{:tag :tr, :attrs nil, :content [{:tag :td, :attrs nil, :content [{:tag :input, :attrs {:name "_INPUT_1_"}, :content nil} " en "]} {:tag :td, :attrs nil, :content [{:tag :i, :attrs nil, :content [" text " {:tag :input, :attrs {:name "_INPUT_2_"}, :content nil} " text"]}]}]}]}]} "\n\n                     " {:tag :img, :attrs {:src "https://docs.google.com/drawings/d/1cP-5PnZA-jvGTDL9D9C6eZt-wxuTLNbKPJRpfU6kxxc/pub?w=735&h=260"}, :content nil} "\n\n" {:tag :p, :attrs {:class "small"}, :content ["Als je aantallen bij elkaar neemt noem je dat " {:tag :b, :attrs nil, :content ["optellen"]} ". " {:tag :br, :attrs nil, :content nil} "\nHet teken " {:tag :b, :attrs nil, :content ["+"]} " noem je " {:tag :b, :attrs nil, :content ["plus"]} " of het plusteken." {:tag :br, :attrs nil, :content nil} "\nMet ‘" {:tag :b, :attrs nil, :content ["de som"]} "’ wordt de uitkomst na het optellen bedoeld."]} "\n\n" {:tag :h1, :attrs nil, :content ["Volgorde van optellen"]} "\n\n" {:tag :p, :attrs nil, :content ["Wanneer je twee getallen optelt maakt de volgorde niet uit:" {:tag :br, :attrs nil, :content nil} "\n" {:tag :span, :attrs {:class "inline_math"}, :content ["5 + 7  =  7 + 5  =  12"]}]} "\n\n" {:tag :img, :attrs {:src "https://docs.google.com/drawings/d/1xYdvVdENGvcrt2fTT8jJT5z1uCdF2awrxmVxP0IDqnc/pub?w=611&h=112"}, :content nil} "\n\n"]} ))

    (let [table-text "<table><tr><td>Answer</td><td>Input</td></tr>
                             <tr><td>123</td><td>_INPUT_1_</td></tr>
                             <tr><td>456</td><td>_INPUT_2_</td></tr></table>"]
      (is (= (material/text-with-inputs-to-tree table-text input-names)
             '{:tag :div, :attrs nil, :content ({:tag :table, :attrs nil, :content ({:tag :tr, :attrs nil, :content ({:tag :td, :attrs nil, :content ("Answer")} {:tag :td, :attrs nil, :content ("Input")})} "\n                             " {:tag :tr, :attrs nil, :content ({:tag :td, :attrs nil, :content ("123")} {:tag :td, :attrs nil, :content ({:tag :input, :attrs {:name "_INPUT_1_"}, :content nil})})} "\n                             " {:tag :tr, :attrs nil, :content ({:tag :td, :attrs nil, :content ("456")} {:tag :td, :attrs nil, :content ({:tag :input, :attrs {:name "_INPUT_2_"}, :content nil})})})})})))))
