class ChapterQuizQuestionsController < ApplicationController
  ...
  def set_param_objects
    ...
    @question = @chapter_questions_set.questions.find_by_uuid(params[:id], false) if params[:id]
  end
  ...
  def set_breadcrumb
    ...
    @crumbs << {name: @question.to_param, url: chapter_chapter_quiz_chapter_questions_set_question_path(@chapter, @chapter_questions_set, @question)} if @question
  end
